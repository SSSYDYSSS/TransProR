...
GetTcgaExp <- function(counts_file_path, gene_probes_file_path, phenotype_file_path, output_file_path) {
  ...
  # Remove duplicates
  count_data_unique <- dplyr::distinct(count_probes_merged, gene, .keep_all = TRUE)
  ...
  tumor_samples <- phenotype_data %>% 
    dplyr::filter(grepl("Metastatic|Primary Tumor", sample_type.samples, ignore.case = TRUE))
  ...
  normal_samples <- phenotype_data %>% 
    dplyr::filter(grepl("normal", sample_type.samples, ignore.case = TRUE))
